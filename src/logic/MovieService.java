...
public class MovieService extends MovieServiceBase {
	 ...
	 */
	public MovieService() {
		// TODONE see for example https://mongodb.github.io/mongo-java-driver/3.12/driver/tutorials/
		// TODONE: connect to MongoDB
		...
		// TODONE Take "movies" and "tweets" collection
	...
	public FindIterable<Document> getBestMovies(int minVotes, double minRating, int limit) {
		//TODONE : implement
		FindIterable<Document>  result = movies.find(and(gte("votes", minVotes), gte("rating", minRating))).limit(limit);
		return result;
	}
